...
#			my $pid = open2 my $from, my $to, "./sha$alg\_bdigest $len";
			my $pid = open2 my $from, my $to,
							"./sha$alg\_btest $len " . length( $msg ) / 2;
			print $to pack $pack_format, $msg;
			close $to;
			...
			chomp $res;
			if ( $res eq $digest ) {
				print "sha$alg\_btest: Ok for LEN = $len\n";
			} else {
				die "sha$alg\_btest: Failure for LEN = $len\n";
			}
		}
	...
	for my $stage ( @stages ) {
		...
		if ( $stage eq $mod[ 2 ] ) {
			print "sha$alg\_btest: Ok COUNT = $count\n";
		} else {
			die "sha$alg\_btest: Failure for COUNT = $count\n"
		}
		$count++;
