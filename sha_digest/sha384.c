/*
 *  This program implements the Secure Hashing Algorithm SHA384 as
 *  described in FIPS PUB 180-3 published in October 2008, see
 *
 *  http://csrc.nist.gov/publications/fips/fips180-3/fips180-3_final.pdf
 *
 *  PLEASE NOTE: While I implemented the SHA384 algorithm as good as my
...
/* Initial hash values (see p. 15 of FIPS 180-3), split up into
   32-bit values */

static const sha_u32 H[ 8 ][ 2 ] = { { 0xcbbb9d5d, 0xc1059ed8 },
...
static void sha384_process_block( SHA384_Context * context );
static void sha384_evaluate( SHA384_Context * context );
...
int
sha384_initialize( SHA384_Context * context )
{
...
int
sha384_add_bytes( SHA384_Context * context,
                  const void     * data,
                  size_t           num_bytes )
{
    ...
    if ( context->off_count != 0 )
        return sha384_add_bits( context, data, 8 * num_bytes );
...
int
sha384_add_bits( SHA384_Context * context,
                 const void     * data,
                 size_t           num_bits )
{
    ...
    {
        int ret = sha384_add_bytes( context, data, num_bits / 8 );
...
int
sha384_calculate( SHA384_Context  * context,
                  unsigned char   digest[ SHA384_HASH_SIZE ] )
{
    ...
    if ( ! context->is_calculated )
        sha384_evaluate( context );

    for ( i = j = 0; j < SHA384_HASH_SIZE; i++ )
    {
...
static void
sha384_process_block( SHA384_Context * context )
{
    size_t         t;
    sha_u64        W[ 80 ];
...
static void
sha384_evaluate( SHA384_Context * context )
{
    int      i;
    sha_u128 count;
