/*
 *  This program implements the Secure Hashing Algorithm SHA224 as
 *  described in FIPS PUB 180-3 published in October 2008, see
 *
 *  http://csrc.nist.gov/publications/fips/fips180-3/fips180-3_final.pdf
 *
 *  PLEASE NOTE: While I implemented the SHA224 algorithm as good as my
...
static const sha_u32 H[ ] = { 0xc1059ed8,
                              0x367cd507,
                              0x3070dd17,
                              0xf70e5939,
                              0xffc00b31,
...
static void sha224_process_block( SHA224_Context * context );
static void sha224_evaluate( SHA224_Context * context );
...
int
sha224_initialize( SHA224_Context * context )
{
    if ( ! context )
...
int
sha224_add_bytes( SHA224_Context * context,
                  const void     * data,
                  size_t           num_bytes )
{
    ...
    if ( context->off_count != 0 )
        return sha224_add_bits( context, data, 8 * num_bytes );
...
int
sha224_add_bits( SHA224_Context * context,
                 const void     * data,
                 size_t           num_bits )
{
    ...
    {
        int ret = sha224_add_bytes( context, data, num_bits / 8 );
...
int
sha224_calculate( SHA224_Context  * context,
                  unsigned char   digest[ SHA224_HASH_SIZE ] )
{
    size_t i,
           j;
    ...
    if ( ! context->is_calculated )
        sha224_evaluate( context );

    for ( i = j = 0; j < SHA224_HASH_SIZE; i++ )
    {
...
static void
sha224_process_block( SHA224_Context * context )
{
    size_t         t;
    sha_u32        W[ 64 ];
...
static void
sha224_evaluate( SHA224_Context * context )
{
    int     i;
    sha_u64 count;
